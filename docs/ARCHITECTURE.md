...
| Charts | **Recharts 3** (web), hand-rolled SVG (mobile) | The portal's `components/charts.tsx` renders on Recharts wearing BoardUI's chart idiom — no axis or tick lines, `preserveStartEnd` ticks, monotone curves, gradient washes, a dashed cursor and a halo active marker — with the same `TimeSeriesPanels` contract the pages always used. BoardUI itself was evaluated and not adopted: it is web-only, its line chart is a paid component, and its CLI installs a second design system. The app cannot run Recharts, so `apps/mobile/src/components/charts.tsx` draws the same idiom on react-native-svg, including a transcribed monotone-cubic curve so a smoothed line cannot invent a peak. Series colours come from the validated palette in `tokens.ts`, mirrored in `globals.css`. |
