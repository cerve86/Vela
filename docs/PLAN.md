...
## Portal — programmes can be deleted; a real exercise library (7 September 2026)

**Deleting a programme.** A Delete button on the programme page and on every row of the
list, one confirmation, and `DELETE /api/programs/{id}` behind it — which is also the
`delete_program` tool in the Claude Desktop extension. A programme nobody was ever
assigned is deleted outright, days and items with it. One a client was ever assigned is
archived instead: the assignment and the sessions on her calendar point at its days, so
deleting it would either be refused by the database or take a live plan off a phone.
Archived, it leaves the list and the API and her week carries on. The response says
which happened.

**The exercise library.** Sixteen hand-written movements were not a library. The Free
Exercise DB (github.com/yuhonas/free-exercise-db, public domain under The Unlicense)
adds 873 more as shipped rows — no coach, so every coach reads them and none edits
them — each with its instructions as cues, its muscles in Vela's words, its equipment,
and a note saying what kind of movement it is (level, compound or isolation, push or
pull). Its categories map onto Vela's: strength, powerlifting, weightlifting and
strongman to strength; stretching to mobility; plyometrics to impact; its fourteen cardio
entries to running, the nearest thing. The three names Vela already had keep their own
rows. Generated by `scripts/exercise-library-migration.py`, upserted by a deterministic
id, so a refresh is a rerun. The programme builder's picker gained a search box, because
nine hundred options is not a select.

## 0.3.2 — Written programmes; the calendar feed explained (7 September 2026)

**A written programme.** Not every plan is days of sets and reps: a physiotherapist often
writes what to do as prose and the client reads it through. `programs.kind` is now
`structured` or `descriptive`, and a descriptive programme has a `body` and no days. In
the portal it is the second choice on the new-programme form, a textarea on its page
with Save and the same Assign card; on the API it is `POST /api/programs` and in the
Claude Desktop extension `create_descriptive_program`, so Francesca's Claude can send a
week's instructions as text. Assigning it runs `assign_program` as before — it replaces
the live programme and clears future scheduled sessions — and puts nothing on the
calendar; instead the client reads the programme she is assigned (a new read policy on
`programs`, tested), and Today shows it as a card above the day's session with the first
lines, opening a reading screen that lays the text out by the shape of its lines alone:
blank lines make paragraphs, numbered or dashed lines a list, a short line ending in a
colon or all in capitals a heading. The text is read live, so an edit after assigning
changes what she sees. App 0.3.2.

**The calendar feed** was checked against sessions three and forty-five days out and
